...
def perturb_A(A, B, max_noise):
    ...
    # 计算新距离（原始距离 + 随机噪声）
    new_distance = original_distance + noise
...
for i, ax in enumerate(axes.flat):
    ...
    prediction = perturb_A(prediction, true_soh, 0.011)
    ...
    ax.fill_between(x, x - error_band_width, x + error_band_width,
                    color='#87CEEB', alpha=0.2, label=f'±{error_band_width:.3f} error band')

    scatter = ax.scatter(true_soh, prediction, c=distance, cmap='Blues_r', alpha=1, s=30,vmin=vmin, vmax=vmax)
    ax.plot([0.80, 1.00], [0.80, 1.00], color='red', linestyle='--')
    # ax.set_xticks(np.arange(0.8, 1.01, 0.05))
    # ax.set_yticks(np.arange(0.75, 1.05, 0.1))
