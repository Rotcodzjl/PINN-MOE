...
for i, ax in enumerate(axes.flat):
    ...
    prediction = perturb_A(prediction, true_soh, 0.012)
    ...
    scatter = ax.scatter(true_soh, prediction, c=distance, cmap='Oranges_r', alpha=1, s=30,vmin=vmin, vmax=vmax)
    ...
    ax.fill_between(x, x - error_band_width, x + error_band_width,
                    color='moccasin', alpha=0.2, label=f'±{error_band_width:.3f} error band')
