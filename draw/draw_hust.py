...
def get_all_data(b_name,dV_threshold=0.000001,window_size=20):
    file_path = source_data_path+b_name+'.pkl'
    file_data=pd.read_pickle(file_path)[b_name]['data']
    data_out=[]
    cap_0=0
    for i in range(1,len(file_data)+1):
        ...
        if i>10:
            data_out.append([I_cv,V_cc,dcdv,cct,cvt,SOH])
    return data_out
DATA_OUT=get_all_data(Battery_list[0])
...
def subtract_all_features(data_out):
    ...
    all_data=detect_and_remove_outliers(all_data, window_size=75)
    print(all_data.shape)
    features=all_data[:,:10]
    SOHs=all_data[:,10]
    return features,SOHs
features,SOHs = subtract_all_features(DATA_OUT)
np.savez("HUST.npz", arr1=features, arr2=SOHs)
