...
def apply_threshold_to_columns(arr, thresholds):
    # 确保阈值列表的长度与数组的列数相同
    if len(thresholds) != arr.shape[1]:
        raise ValueError("阈值列表的长度必须与数组的列数相同")
    ...
    for col_index in range(arr.shape[1]):
        ...
        for row_index in range(1, arr.shape[0]):
            # 计算当前元素与前一个元素的差值
            diff = abs(arr[row_index, col_index] - arr[row_index - 1, col_index])
            # 如果差值大于阈值，则将当前元素设置为前一个元素的值
            if diff > threshold:
...
def multiply_column(arr, column_index, multiplier):
    """
    Multiply all elements in a specified column of a NumPy array by a given number.

    Parameters:
    arr (np.ndarray): The input NumPy array.
    column_index (int): The index of the column to multiply.
    multiplier (int or float): The number to multiply the column elements by.

    Returns:
    np.ndarray: The array with the specified column multiplied.
    """
    arr[:, column_index] *= multiplier
    return arr


def subtract_features(data_out):
    features = []
    SOHs = []
    out = []
    for i in range(len(data_out)):
        I_cv, V_cc, dcdv, CCT, CVT, SOH = data_out[i][0], data_out[i][1], data_out[i][2], data_out[i][3], data_out[i][
            4], data_out[i][5]
        """电压的样本熵"""
        en_v = 1000 * SampleEntropy(V_cc, r=1.5)
        """电压的功率谱密度"""
        ...
        max_ic = max(dcdv) / 1e8
        out.append(CVT)
        features.append([en_v, psd_v, psd_ic, max_ic, CCT, CVT])
        SOHs.append(SOH)
    features = np.stack(features, axis=0)
    SOHs = np.stack(np.reshape(SOHs, (-1, 1)), axis=0)
    all_data = np.concatenate((features, SOHs), axis=1)
    all_data = apply_threshold_to_columns(all_data, [0.5, 5, 4, 2, 10, 200, 20])
    all_data = smooth_and_normalize_columns(all_data, window_size=3)
    all_data = multiply_column(all_data, column_index=3, multiplier=1e-7)
    all_data = multiply_column(all_data, column_index=2, multiplier=1e-7)
    all_data = multiply_column(all_data, column_index=1, multiplier=1e-7)
    all_data = multiply_column(all_data, column_index=0, multiplier=1e-7)
    features = all_data[:, :6]
    SOHs = all_data[:, 6]
    return features, SOHs

def get_all_data(batch_name,dV_threshold=0.000001,window_size=20):
    ...
    for Battery_name in Battery_list:
        ...
        for i in range(60,len(data_all)):
            ...
            I_cv=df_cv[(df_cv['i']<=0.5) & (df_cv['i']>=0.1)]['i'].tolist()
            """cccv图"""
            C=df_cc['c'].tolist()
            V=df_cc['v'].tolist()
    ...
    return data_batch
DATA_OUT=get_all_data(Batch_list[0],dV_threshold=0.000001,window_size=20)
...
def subtract_all_features(data_out):
    ...
    for i in range(len(data_out)):
        ...
        std_v=np.std(V_cc)
        """电流样本熵"""
        en_c=SampleEntropy(I_cv,m=3,r=0.2)
        """电流的功率谱密度"""
    ...
    SOHs=np.stack(np.reshape(SOHs,(-1,1)),axis=0)
    all_data=np.concatenate((features,SOHs),axis=1)
    all_data=smooth_and_normalize_columns(all_data,window_size=19)
    all_data=detect_and_remove_outliers(all_data, window_size=25)
    # all_data=multiply_column(all_data,column_index=3,multiplier=1e-7)
    features=all_data[:,:10]
    SOHs=all_data[:,10]
    return features,SOHs
features,SOHs = subtract_all_features(data)
np.savez("xjtu.npz", arr1=features, arr2=SOHs)
